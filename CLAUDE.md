...
- Never write bare `except Exception` that silently returns/continues. Fail-open is required for third-party checks (an API outage must not degrade the score), but catch only the expected exception types (httpx.HTTPError, dns.exception.DNSException, ValueError, OSError, ...) and emit a logger.warning/debug for each indeterminate check so outages and refactor bugs stay visible.
